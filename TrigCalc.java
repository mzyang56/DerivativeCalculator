...
public class TrigCalc extends DerivativeCalc {
    
    //Constructor
    public TrigCalc(String uInput) {
        super(uInput);
    }

    //Overrides the calculate method in the DerivativeCalc class to include trigonometric differentiation
    public List<String> calculate(List<String> equation, List<String> equation2) {
            while (equation.contains("o") || equation.contains("sinq") || equation.contains("cosq")) {
    
                if (equation.contains("sinq")) {
                    int sinPos = equation.indexOf("sinq");
                    equation.set(sinPos, "cosx");
                }
                
                if (equation.contains("cosq")) {
                    int cosPos = equation.indexOf("cosq");
                    equation.set(cosPos, "-sinx");
                }
    ...
    //Overrides the convert method in the DerivativeCalc class
    public void convertToList(String input, List<String> eq1, List<String> eq2) {
        //Variable that when true creates a second array that will not be differentiated due to the chain rule
            ...
            for (int i = 0; i < input.length(); i++) {
                ...
                try {
                    if (input.substring(i, i+3).equals("sin") || input.substring(i, i+3).equals("cos")) {
                        eq1.add(input.substring(i, i+3)+"q");
                        isTrig = true;
                        if (innerList == true) {
                            eq2.add(input.substring(i,i+3)+"x");
                        }
                        //+3 to also skip over adding the 'x'
                        i += 3;
                    }
                ...
                if (isTrig == false) {
                    ...
                    if (input.substring(i,i+1).equals("x")) {
                        ...
                        //Stops code if character is not part of sin, cos, or an expression and is also not an integer
